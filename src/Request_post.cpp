...
void		Server::set_content_post(std::string full_msg) {
	...
	full_msg.erase(0, full_msg.size() - found1);
	found1 = full_msg.find("\r\n", 0);
	boundary = full_msg.substr(10, found1);
	found1 = full_msg.find("Content-Length:", 0);
	full_msg.erase(0, full_msg.size() - found1);
	found1 = full_msg.find("\r\n", 0);
	content_lenght = full_msg.substr(16, found1);
	found1 = full_msg.find("\r\n", 0);
	found1 = content.find(boundary + "\r\n", 0);
	found2 = content.find(boundary + "--" + "\r\n", 0);
	...
	request_post(6, _Post_file_name);
